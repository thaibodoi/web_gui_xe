...
public class ConfigServiceImpl implements ConfigService {
    ...
    public ShiftConfigResponse updateShiftConfig(ShiftConfigRequest request) {
        ...
        // Nếu GIẢM giới hạn: chặn nếu vẫn còn thẻ vượt mức (cardId > newMax) đang có xe đỗ.
        // (Giới hạn sẽ được áp dụng khi quẹt xe vào: thẻ vượt giới hạn sẽ không cho dùng.)
        if (newMax < currentMax) {
            boolean someExcessCardInUse = parkingRecordRepository.existsByCard_CardIdGreaterThan(newMax);
            if (someExcessCardInUse) {
                throw new AppException(ErrorCode.PARKING_CARD_LIMIT_IN_USE);
            }
        }
        ...
        // TĂNG giới hạn: sinh thêm thẻ trong dải (currentMax+1 .. newMax).
        // GIẢM giới hạn: KHÔNG xóa thẻ cũ (vì thẻ đã dùng còn được lịch sử gửi xe tham chiếu
        // -> xóa sẽ vi phạm khóa ngoại). Giới hạn được áp dụng ở bước quẹt xe vào.
        if (newMax > currentMax) {
